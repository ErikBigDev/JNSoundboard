...
namespace JNSoundboard
{
	public partial class MainForm : Form
	{
		...
		public MainForm()
		{
			...
			if (cbPlaybackDevicesLocal.Items.Contains(XMLSettings.soundboardSettings.LastPlaybackDeviceLocal)) cbPlaybackDevicesLocal.SelectedItem = XMLSettings.soundboardSettings.LastPlaybackDeviceLocal;
			...
			volumeBar.Value = XMLSettings.soundboardSettings.Volume;
			AudioPlaybackEngine.Instance.Volume = volumeBar.Value / 100.0f;

			//add events after settings have been loaded
			cbPlaybackDevicesLocal.SelectedIndexChanged += cbPlaybackDevicesLocal_SelectedIndexChanged;
		...
		private void initAudioPlaybackEngine()
		{
			...
			int localDeviceNumber = cbPlaybackDevicesLocal.SelectedIndex;

			try
			{
				AudioPlaybackEngine.Instance.Init(localDeviceNumber, deviceNumber);
			}
		...
		private void loadSoundDevices()
		{
			...
			if (cbPlaybackDevicesLocal.Items.Count > 0)
				cbPlaybackDevicesLocal.SelectedIndex = 0;
		...
		private void playSound(string file)
		{
			...
			try
			{
				if(cbEnableVirtual.Checked)
					AudioPlaybackEngine.Instance.PlaySoundVirtual(file);
				if(cbEnableLocal.Checked)
					AudioPlaybackEngine.Instance.PlaySoundLocal(file);
			}
		...
		{
			Process.Start("https://github.com/ErikBigDev/JNSoundboard/releases");
		}
		...
		private void cbPlaybackDevicesLocal_SelectedIndexChanged(object sender, EventArgs e)
		{
			//start loopback on new device and stop all sounds playing
			if (loopbackWaveOut != null && loopbackSourceStream != null && cbEnable.Checked)
				startLoopback();
			...
			string deviceName = (string)cbPlaybackDevicesLocal.SelectedItem;
			XMLSettings.soundboardSettings.LastPlaybackDeviceLocal = deviceName;
		...
		private void volumeBar_Scroll(object sender, EventArgs e)
		{
			...
			XMLSettings.soundboardSettings.Volume = volumeBar.Value;
			XMLSettings.SaveSoundboardSettingsXML();
		}

		private void cbEnableLocal_CheckedChanged(object sender, EventArgs e)
		{
			cbPlaybackDevicesLocal.Enabled = cbEnableLocal.Checked;
		}

		private void cbEnableVirtual_CheckedChanged(object sender, EventArgs e)
		{
			cbPlaybackDevices.Enabled = cbEnableVirtual.Checked;
		}
	}
}
